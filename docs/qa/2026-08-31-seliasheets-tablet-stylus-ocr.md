...
Android 10's render helper drops immediate first-contact input unless the canvas initializes after attachment and cannot safely reuse its detached render helper. API 29 now warms the attached canvas and replaces only that helper after detach. Newer Android versions retain lazy hover/contact initialization to avoid repeated renderer startup during recreation.
